...
##						                        ##
##	     DEFAULT BACKUP PRE ACTION SCRIPT		##
##      		      ©2005			            ##
##						                        ##
##		           Version 0.1    	            ##
##                                              ##
...
ACTION_DIRECTORY="${backupConfigurationFolderPath}/resources/pre-actions"

# Actions names must be executable and pre-fixed with the ACTION_PREFIX value
ACTION_PREFIX="BACKUP"
...
if [ -d "${ACTION_DIRECTORY}" ]; then
    ...
    for action in "${ACTION_DIRECTORY}/${ACTION_PREFIX}"* ; do
        ...
        if [ -s "${action}" -a -x "${action}" ]; then
            ...
            # Write Pre Backup Action Detils to Log
            echo "Performing Pre-Action : ${action_name}" | tee -ai $logFile
